...
# ── Helper: strip HTML tags AND stray attribute fragments like  foo=" /> ────
function Remove-HtmlMarkupStrict {
    param([string]$Html)
    # First remove full tags
    $t = [regex]::Replace($Html, '<[^>]*>', '')
    # Remove residual attribute junk: anything starting with  "  or ending with  "/>  or  />
    $t = $t -replace '(?s)"\s*/>', ''
    $t = $t -replace '(?s)/>', ''
    # Decode entities
    $t = $t -replace '&amp;',  '&'
...
# ── Helper: extract a clean CU/SP/GDR label from any text ────────────────────
function Get-PatchLabel {
    param([string]$Text)
    # Try most-specific patterns first (CU+GDR before bare GDR, etc.)
    $patterns = @(
        '(?i)(RTM\s*[\+\-]\s*GDR)',
        '(?i)(CU\s*\d+\s*[\+\-]\s*GDR)',
        '(?i)(SP\s*\d+\s*CU\s*\d+\s*[\+\-]\s*GDR)',
        '(?i)(SP\s*\d+\s*CU\s*\d+)',
        '(?i)(CU\s*\d+)',
        '(?i)(SP\s*\d+)',
        '(?i)(RTM)',
    ...
    foreach ($p in $patterns) {
        ...
        if ($m.Success) {
            return ($m.Groups[1].Value -replace '\s+', '' -replace '\+', '+').ToUpper()
        }
    }
    return ''
}
...
function Get-BuildsFromBlogspot {
    ...
    $bestVer  = @{}   # maj → [System.Version] – highest CU build seen
    $bestGDR  = @{}   # maj → [System.Version] – highest GDR build seen (tracked separately)

    $rowPattern  = '(?si)<tr\b[^>]*>(.*?)</tr>'
    $cellPattern = '(?si)<t[dh]\b[^>]*>(.*?)</t[dh]>'

    foreach ($row in [regex]::Matches($html, $rowPattern)) {
        # Use strict cleanup on each cell to avoid attribute-text leakage
        $cells = @([regex]::Matches($row.Groups[1].Value, $cellPattern) |
                   ForEach-Object { Remove-HtmlMarkupStrict $_.Groups[1].Value })

        if ($cells.Count -lt 3) { continue }

        # ── Build number: first X.X.X[.X] token in any of the first 3 cells ──
        $buildStr = $null
        foreach ($ci in 0,1,2) {
            if ($ci -ge $cells.Count) { break }
            $bm = [regex]::Match($cells[$ci], '(\d{1,2}\.\d{1,4}\.\d{1,6}(?:\.\d{1,6})?)')
            if ($bm.Success) { $buildStr = $bm.Groups[1].Value; break }
        }
        if (-not $buildStr) { continue }
        ...
        # ── KB number: scan ALL cells for KBxxxxxxx ──────────────────────────
        $kbNumber = ''
        foreach ($cell in $cells) {
            $kbm = [regex]::Match($cell, '(?i)KB\s*(\d{6,8})')
            if ($kbm.Success) { $kbNumber = "KB$($kbm.Groups[1].Value)"; break }
        }
        # Fallback: bare 7-digit number in cell[2]
        if (-not $kbNumber -and $cells.Count -ge 3) {
            $kbm = [regex]::Match($cells[2], '(\d{7})')
            if ($kbm.Success) { $kbNumber = "KB$($kbm.Groups[1].Value)" }
        }
        ...
        # ── Patch label: search ALL cells, most-specific wins ────────────────
        $patchLabel = ''
        $allCellText = $cells -join ' '
        $patchLabel = Get-PatchLabel -Text $allCellText

        # ── Determine if this is a GDR-only build ────────────────────────────
        $isGDROnly = ($patchLabel -eq 'GDR') -or ($allCellText -imatch '\bGDR\b' -and $allCellText -inotmatch '\bCU\s*\d')
        ...
        # ── Description: use the cell with the most meaningful text ──────────
        $descRaw = ''
        foreach ($cell in ($cells | Sort-Object Length -Descending)) {
            $candidate = $cell -replace '(?i)\s+or\s+\d[\d.]+.*$', ''
            $candidate = [regex]::Replace($candidate, '\d{1,2}\.\d+\.\d+[\d.]*', '').Trim()
            $candidate = ($candidate -replace '\s+', ' ').Trim()
            # Skip cells that are just a KB number or very short
            if ($candidate.Length -gt 8 -and $candidate -notmatch '^KB\d') {
                $descRaw = $candidate
                break
            }
        }
        if ($descRaw.Length -gt 120) { $descRaw = $descRaw.Substring(0, 117) + '...' }

        $productName = if ($cells[0] -match '(SQL\s*Server\s*\d{4}(?:\s*R2)?)') {
            $Matches[1] -replace '\s+', ' '
        } else { "SQL Server" }

        # ── Track best build per major version ───────────────────────────────
        # RULE: Always prefer a CU over a GDR of the same or lower build number.
        # Only update the "best" entry if:
        #   (a) This is a CU build and it's higher than the current best CU, OR
        #   (b) This is a GDR build but no CU has been seen yet, and it's higher than the current best.
        $updateBest = $false
        if (-not $isGDROnly) {
            # CU (or RTM) build – always preferred
            if (-not $bestVer.ContainsKey($maj) -or $buildVer -gt $bestVer[$maj]) {
                $bestVer[$maj] = $buildVer
                $updateBest = $true
            }
        } else {
            # GDR-only build – only use if no CU entry exists at all yet
            $bestGDR[$maj] = if (-not $bestGDR.ContainsKey($maj) -or $buildVer -gt $bestGDR[$maj]) { $buildVer } else { $bestGDR[$maj] }
            if (-not $bestVer.ContainsKey($maj) -and (-not $builds.ContainsKey($maj) -or $buildVer -gt ([System.Version]$builds[$maj].LatestBuild))) {
                $updateBest = $true
            }
        }
        ...
        if ($updateBest) {
            $builds[$maj] = @{
                ...
                Description = "$productName – $descRaw".Trim(' –')
                KBUrl       = $kbUrl
                KBNumber    = $kbNumber
                PatchLabel  = $patchLabel
            }
...
function Get-BuildsFromMicrosoftLearn {
    ...
    foreach ($kv in $learnPages.GetEnumerator()) {
        ...
        # Description: find the row in the table that corresponds to the highest build we found,
        # then extract its CU/GDR label cleanly. Fall back to scanning for the first CU mention.
        $desc = 'Latest update'
        $descMatch = [regex]::Match($pageHtml,
            '(?i)(Cumulative\s+Update\s+\d+(?:\s*\([^)]*\))?(?:\s*[\+\-]\s*GDR)?|GDR(?:[^<]{0,60})?|Service\s+Pack\s+\d+(?:\s*CU\s*\d+)?)')
        if ($descMatch.Success) {
            $desc = ($descMatch.Groups[1].Value -replace '<[^>]*>', '' -replace '\s+', ' ').Trim()
            # Strip any trailing HTML attribute leakage
            $desc = ($desc -replace '"[^"]*/?>', '' -replace '/>', '').Trim()
        }

        # KB number: look for KBxxxxxxx near the highest build number in the page
        $kbNumberS2 = ''
        $kbNumMatch2 = [regex]::Match($pageHtml, '(?i)KB\s*(\d{6,8})')
        if ($kbNumMatch2.Success) { $kbNumberS2 = "KB$($kbNumMatch2.Groups[1].Value)" }

        # KB link: any support.microsoft.com link on the page
        $kbMatch = [regex]::Match($pageHtml, 'href="(https://support\.microsoft\.com/[^"]+(?:kb|help)[^"]*)"')
            ...
            foreach ($k in $s2.Keys) {
                if ($combined.ContainsKey($k)) {
                    try {
                        ...
                        # Determine if each is GDR-only
                        $label1 = if ($combined[$k].PatchLabel) { $combined[$k].PatchLabel } else { $combined[$k].Description }
                        $label2 = if ($s2[$k].PatchLabel)       { $s2[$k].PatchLabel }       else { $s2[$k].Description }
                        $is1GDR = ($label1 -imatch '\bGDR\b' -and $label1 -inotmatch '\bCU\s*\d')
                        $is2GDR = ($label2 -imatch '\bGDR\b' -and $label2 -inotmatch '\bCU\s*\d')

                        # Prefer CU over GDR regardless of build number.
                        # If both are CU or both are GDR, take the higher build.
                        $useS2 = $false
                        if     (-not $is2GDR -and $is1GDR)  { $useS2 = $true }   # S2=CU beats S1=GDR
                        elseif (-not $is1GDR -and $is2GDR)  { $useS2 = $false }  # S1=CU beats S2=GDR
                        elseif ($v2 -gt $v1)                 { $useS2 = $true }   # same type – higher wins

                        if ($useS2) {
                            # Carry KBNumber from Source 1 map if Source 2 didn't get one
                            if (-not $s2[$k].KBNumber -and $combined[$k].KBNumber) {
                                $s2[$k].KBNumber = $combined[$k].KBNumber
                            }
                            $combined[$k] = $s2[$k]
                            Write-Log "  [Merge] v$($k): MS Learn ($($s2[$k].LatestBuild)) preferred over blogspot ($v1)."
                        } else {
                            # Keep S1 but fill in KBUrl/KBNumber from S2 if missing
                            if (-not $combined[$k].KBUrl    -and $s2[$k].KBUrl)    { $combined[$k].KBUrl    = $s2[$k].KBUrl }
                            if (-not $combined[$k].KBNumber -and $s2[$k].KBNumber) { $combined[$k].KBNumber = $s2[$k].KBNumber }
                        }
