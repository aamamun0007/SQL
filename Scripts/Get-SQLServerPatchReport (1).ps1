...
function Build-HtmlReport {
    ...
    $rows = foreach ($r in $Results) {
        ...
        # Patch-to-apply KB cell – separate from the "current" KB
        $patchKbHtml = if ($r.KBUrl -and $r.PatchStatus -eq 'PATCH AVAILABLE') {
            "<a href='$($r.KBUrl)' style='color:#0d47a1;'>KB Article</a>"
        } elseif ($r.KBUrl) {
            "<a href='$($r.KBUrl)' style='color:#0d47a1;'>KB Article</a>"
        } else { '—' }
